...
def getEdit(model,enc_inputs,dec_inputs,device):
...
#
def getEdit_batch(model, enc_batch, dec_batch):
    """
    enc_batch: [B, L, D]
    dec_batch: [B, L, D]
    """
    # 将输入处理成三维的 变成一批一批的，然后输入到模型中进行预测
    enc_batch = enc_batch.cuda()
    dec_batch = dec_batch.cuda()

    allpredict, _, _, _ = model(enc_batch, dec_batch)
    allpredict1, _, _, _ = model(dec_batch, dec_batch)
    allpredict = allpredict + allpredict1  # shape: [B*L, vocab_size] if flatten

    predict = allpredict.data.max(1, keepdim=True)[1]  # shape: [B*L, 1]
    ...
    predict = predict.view(B, L)
    allpredicts = allpredicts.view(B, L, -1).cpu().detach().numpy()

    dna_sequence_pulsdel_list = []
    prewith_phred_pulsdel_list = []
    prewith_seq_list = []
    prewith_phred_list = []

    for i in range(B):
        pre = convert_to_DNA(predict[i])
        prewith_seq, prewith_phred, _ = convert_to_DNA_(predict[i], allpredicts[i])
        dna_sequence_pulsdel, prewith_phred_pulsdel = converttoDNAgetdel(predict[i], allpredicts[i], 0)
        ...
        dna_sequence_pulsdel_list.append(dna_sequence_pulsdel)
        prewith_phred_pulsdel_list.append(prewith_phred_pulsdel)
        prewith_seq_list.append(prewith_seq)
        prewith_phred_list.append(prewith_phred)

    return dna_sequence_pulsdel_list, prewith_phred_pulsdel_list, prewith_seq_list, prewith_phred_list

def testnet(model,enc_inputstest, dnatest_phreds,testseqs,testphreds,device):
    pre_seqs = []
    phred_scores = []
    prewith_seqs = []
    phredwith_scores = []
    ...
    for i in range(len(enc_inputstest)):
        # if i%100==0:print(i)
        dna_sequence_pulsdel, prewith_phred_pulsdel,prewith_seq,prewith_phred = getEdit(model, enc_inputstest[i], dnatest_phreds[i],device)
...
def testnet_batch(model, enc_inputstest, dec_inputstest, testseqs, testphreds):
    pre_seqs = []
    phred_scores = []
    prewith_seqs = []
    phredwith_scores = []

    # enc_batch = torch.stack(enc_inputstest)
    # dec_batch = torch.stack(dec_inputstest)

    dna_seqs, phred_pulsdels, prewith_seqs_, prewith_phreds = getEdit_batch(model, enc_inputstest, dec_inputstest)

    for i in range(len(enc_inputstest)):
        if len(dna_seqs[i]) > sequence_length * 1.1:
            dna_seqs[i] = testseqs[i][0]
            phred_pulsdels[i] = ''

        score = ' '.join(str(np.max(p)) for p in phred_pulsdels[i]) if phred_pulsdels[i] else ''
        score_with = ' '.join(f'{j}:{np.max(p)}' for j, p in enumerate(prewith_phreds[i])) if prewith_phreds[i] else ''

        pre_seqs.append(dna_seqs[i])
        phred_scores.append(score)
        prewith_seqs.append(prewith_seqs_[i])
        phredwith_scores.append(score_with)
