...
from polls.Code.reconstruct.dpconsensus.config import *
...
class Encoder(nn.Module):
    ...
    def forward(self, enc_inputs,enc_inputs1=None):
        tensor = torch.randn(enc_inputs.shape[0], enc_inputs.shape[1]).to(device)
...
class Transformer(nn.Module):
    ...
    # def __init__(self):
        ...
        # self.Encoder = Encoder(n_layers).to(device)
        # self.Decoder = MyLSTM()
    ...
    def forward(self, enc_inputs, dec_inputs=None):
        ...
        # merge_outputs = nn.LayerNorm(d_model).to(device)(enc_outputs+dec_outputs)
        # #①0.24 0.9992
...
def getEdit(model,enc_inputs,dec_inputs,dec_outputs):
    ...
    # predict_dec_input = mytest(model, enc_inputs.view(1, -1).to(device))
    # predict, _, _, _ = model(enc_inputs.view(1, -1).to(device),predict_dec_input)
    # predict, _, _, _ = model(enc_inputs.view(1, -1).to(device),enc_inputs.view(1, -1).to(device))
    allpredict, _, _, _ = model(enc_inputs.view(1, enc_inputs.shape[0],-1).to(device),dec_inputs.view(1, dec_inputs.shape[0],-1).to(device))
    allpredict1, _, _, _ = model(dec_inputs.view(1, dec_inputs.shape[0],-1).to(device),dec_inputs.view(1, dec_inputs.shape[0],-1).to(device))
    ...
    # predict, _, _, _ = model(enc_inputs.to(device),enc_inputs.to(device))
