...
def bsalign_alitest22(cluster_seqs, num, suffix=None):
    """
    多线程安全版本的 bsalign 调用
    每次运行生成临时文件避免冲突
    ...
    - cluster_seqs: 当前序列簇
    - num: 序列数
    - suffix: 唯一标识（每次调用必须不同），None 时自动生成
    """

    if suffix is None:
        suffix = f"{os.getpid()}_{uuid.uuid4().hex}"
    else:
        suffix = f"{suffix}_{uuid.uuid4().hex}"
    fasta_path  = f"{dpconsensus_path}/seqs_{suffix}.fasta"
    consus_path = f"{dpconsensus_path}/consus_{suffix}.txt"
    ali_path    = f"{dpconsensus_path}/ali_{suffix}.ali"

    # 保存序列到临时文件
    save_seqs(cluster_seqs, fasta_path)

    # 构造 shell 命令
    shell = (
        f"polls/Code/reconstruct/bsalign/bsalign poa {fasta_path} "
        f"-o {consus_path} -L > {ali_path}"
    )
    result = subprocess.run(shell, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)

    # 判断是否生成了合法的 ali 文件
    if not os.path.exists(ali_path) or os.path.getsize(ali_path) == 0:
        print(f"[Error] bsalign failed for suffix {suffix}")
        print("stderr:", result.stderr)
        print("stdout:", result.stdout)
        for p in (fasta_path, consus_path, ali_path):
            if os.path.exists(p): os.remove(p)
        return [], '', [], ''

    # 读取对齐文件
    seqs, _, aliconsus, errors = read_alifilestest(ali_path, num)

    # 读取共识序列
    with open(consus_path, 'r') as file:
        lines = file.readlines()
        consus = lines[0].strip() if lines else ''
    ...
    for i in range(len(seqs[0])):
        ...
        max_v2 = sorted(d.values())[-2]
        max_k2 = next(k for k, v in d.items() if v == max_v2)
        if max_k == '-' and max_k2 == consus[i]:
            continue
        else:
            break
    newconsus = consus[i:]
    ...
    for i in range(len(seqs[0]) - 1, 0, -1):
        ...
        if max_k == '-':
            if d[max_k] == num:
                continue
            elif max_k2 == newconsus[newlastindex]:
                newlastindex -= 1
                continue
        else:
            break
...
def quasaliseqs(quas, seqs_up, copy_num=10):
    """
    向量化实现，不改变输出，与原 quasaliseqs 结果完全一致。
    quas     : list[list[float]]  每条的 phred/概率列表
    seqs_up  : list[str]          对齐后序列（带 '-')
    copy_num : int                原逻辑中的阈值
    """
    if len(quas) == 0:
        return quas

    # ---------- ① 构造 gap 掩码 ----------
    seq_arr = np.frombuffer("".join(seqs_up).encode("ascii"), dtype="S1")
    seq_arr = seq_arr.reshape(len(seqs_up), -1)       # (M, L)
    gap_mask = (seq_arr == b'-')                      # True 表示 gap

    # ---------- ② 把 quas 填入矩阵 ----------
    L = seq_arr.shape[1]
    M = len(quas)

    # pad quas 到同一长度 L，pad 值用 1（原逻辑缺位时填 1）
    quas_pad = np.ones((M, L), dtype=float)
    for i, q in enumerate(quas):
        quas_pad[i, :len(q)] = q

    # gap → 0，其余 → quas 值
    new_quas = np.where(gap_mask, 0, quas_pad)        # (M, L)

    # ---------- ③ 列级别统计 ----------
    non_zero_cnt = (new_quas != 0).sum(axis=0)        # (L,)
    col_sum      = new_quas.sum(axis=0)               # (L,)

    # 对于 num==0 的列，后面要整体填 1
    need_fill_all1 = (non_zero_cnt == 0)

    # ---------- ④ 处理缺口按原逻辑插值 ----------
    # 建一个布尔索引：哪些元素是 0 且列非全 0 且 num<copy_num
    mask_zero      = (new_quas == 0)
    mask_rare_col  = (non_zero_cnt > 0) & (non_zero_cnt < copy_num)
    mask_update    = mask_zero & mask_rare_col                # (M, L)

    # 右左窗口 (±4) 的邻域平均
    kernel = np.ones(9)
    smoothed = np.convolve(col_sum / np.maximum(non_zero_cnt, 1),  # 均值
                           kernel, mode='same') / kernel.sum()

    # 广播写回
    new_quas[mask_update] = np.take(smoothed, np.where(mask_update)[1])

    # 列全 0 的直接填 1
    new_quas[:, need_fill_all1] = 1.0
...
def promaxPhreddata(encoded_sequences, quas):
    """
    encoded_sequences: shape (N, L, 5)  —— one‑hot 碱基编码
    quas            : shape (N, L)      —— 质量值
    return          : list of length L，每项是 5 个累加得分
    """
    if len(encoded_sequences) == 0:
        return []

    # 保证都是 ndarray
    enc = np.asarray(encoded_sequences, dtype=float)   # (N, L, 5)
    q   = np.asarray(quas,              dtype=float)   # (N, L)

    # 广播 q 到 (N, L, 1)，逐元素乘，再沿 read 维求和
    #   result shape: (L, 5)
    scores = (enc * q[:, :, None]).sum(axis=0)

    # 转回 list[list[float]]、保持原函数的返回格式
...
def process_block(dna_seqs_block, all_quas_block):
    block_all_consus = []
    block_all_seqs_up = []
    block_all_ali_seqs_formaxlen = []
    block_new_dna_sequences = []
    block_new_all_quas = []
    last_consus = last_seqs_up = last_len = None


    tid = threading.get_ident()        # 当前线程唯一 ID
    pid = os.getpid()                   # 当前进程 ID
    for seqsi, seqs in enumerate(dna_seqs_block):

        if seqsi % 1000 == 0:
            print(f"[PID {pid}][Thread {tid}] Block processing seqsi {seqsi}")
        if len(seqs) <= 1:
            print(f"[PID {pid}][Thread {tid}] Warning: seqsi {seqsi} length <= 1")

            # 给每次调用一个唯一后缀：<进程ID>_<线程ID>_<序号>
        suffix = f"{pid}_{tid}_{seqsi}"
        seqs_up, consus, consusno_, _ = bsalign_alitest22(seqs, len(seqs), suffix=suffix)


        if seqs_up == []:
            print(f"[PID {pid}][Thread {tid}] [Skip] seqsi {seqsi} bsalign failed, using last valid result")
            if last_consus is not None and last_seqs_up is not None:
                block_all_consus.append(last_consus)
                block_all_seqs_up.append(last_seqs_up)
                block_all_ali_seqs_formaxlen.append(last_len)
                block_new_dna_sequences.append(dna_seqs_block[seqsi - 1])
                block_new_all_quas.append(all_quas_block[seqsi - 1])
            else:
                print(f"[PID {pid}][Thread {tid}] [Fatal] seqsi {seqsi} no valid fallback, skipping")
            continue

        # 正常记录
        block_all_consus.append(consusno_)
        block_all_seqs_up.append(seqs_up)
        block_all_ali_seqs_formaxlen.append(len(seqs_up[0]))
        block_new_dna_sequences.append(seqs)
        block_new_all_quas.append(all_quas_block[seqsi])
        last_consus, last_seqs_up, last_len = consusno_, seqs_up, len(seqs_up[0])

    return (block_all_consus,
            block_all_seqs_up,
            block_all_ali_seqs_formaxlen,
            block_new_dna_sequences,
            block_new_all_quas)
def process_block_nophred(dna_seqs_block):
    block_all_consus = []
    block_all_seqs_up = []
    block_all_ali_seqs_formaxlen = []
    block_new_dna_sequences = []
...
def trans_seq_hottest(dna_sequences,ori_all_quas,copy_num=10):
    ...
    last_consus = None
    last_seqs_up = None
    last_len = None  # 对应 all_ali_seqs_formaxlen
    starttime1 = datetime.now()
    import math

    num_workers = 16
    # 根据线程来分配每一个线程处理的数据大小，这里的数据是选取了copy_num之后的文件

    block_size = math.ceil(len(dna_sequences) / num_workers)

    blocks = [
        (dna_sequences[i * block_size: (i + 1) * block_size], all_quas[i * block_size: (i + 1) * block_size])
        for i in range(num_workers)
    ]

    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        results = list(executor.map(lambda args: process_block(*args), blocks))
    ...
    for r in results:
        block_all_consus, block_all_seqs_up, block_all_ali_seqs_formaxlen, block_new_dna_sequences, block_new_all_quas = r
        all_consus.extend(block_all_consus)
        all_seqs_up.extend(block_all_seqs_up)
        all_ali_seqs_formaxlen.extend(block_all_ali_seqs_formaxlen)
        new_dna_sequences.extend(block_new_dna_sequences)
        new_all_quas.extend(block_new_all_quas)

    print(f"bsalign花费了：{ str(datetime.now() - starttime1)}")
    dna_sequences = new_dna_sequences
    all_quas = new_all_quas
    sequence_length = max(all_ali_seqs_formaxlen)
    ...
    print(f"独热编码花费了：{str(datetime.now() - starttime2)}")
    print('testdata done!')
...
def trans_seq_hottest_nophred_fast(dna_sequences,copy_num=10):
    dna_sequences = getRadomSeqsNoQua(dna_sequences,copy_num)
    # current_directory = os.getcwd()
    # print("当前工作目录:", current_directory)
    # save_seqs(dna_sequences,dpconsensus_path+'/alltestseqs.fasta')
    ...
    blocks = [
        (dna_sequences[i * block_size: (i + 1) * block_size])
        for i in range(num_workers)
    ]

    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        results = list(executor.map(process_block_nophred, blocks))
    ...
    for r in results:
        block_all_consus, block_all_seqs_up, block_all_ali_seqs_formaxlen, block_new_dna_sequences = r
        all_consus.extend(block_all_consus)
        all_seqs_up.extend(block_all_seqs_up)
        all_ali_seqs_formaxlen.extend(block_all_ali_seqs_formaxlen)
        new_dna_sequences.extend(block_new_dna_sequences)
    ...
    # return all_base_proportions,all_consus
    # save_seqsandoritst(all_seqs_up,dna_sequences,all_consus,dpconsensus_path+'/testori_ali_consus_seqs.fasta')
    return all_base_proportions,all_consus,dna_sequences,all_seqs_up
