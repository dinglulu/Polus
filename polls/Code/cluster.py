...
# ./bwa index /home1/hongmei/00work_files/0000/0isecondwork/Evaluation_platform/POLUS/files/index_DNAFountain.fasta
# ./bwa mem /home1/hongmei/00work_files/0000/0isecondwork/Evaluation_platform/POLUS/files/index_DNAFountain.fasta   /home1/hongmei/00work_files/0000/0isecondwork/Evaluation_platform/POLUS/files/simu/simulated_seqsr1r2.fasta  > /home1/hongmei/00work_files/0000/0isecondwork/Evaluation_platform/POLUS/files/decode/ali.sam
# cd /home1/hongmei/00work_files/0000/0isecondwork/Evaluation_platform/POLUS/files/decode
...
def cluster_by_ref(file_ref,file_seqs,lens,save_path,dir='/home1/hongmei/00work_files/0000/0isecondwork/Evaluation_platform/POLUS/files/decode/'):
    print(f"--------开始聚类cluster_by_ref--------")
    starttime = datetime.now()
    # 使用ref进行聚类，不用区分fasta/fastq文件，都可以使用bwa进行聚类，在读取比对好后的ali文件不同，有phred的需要读取到来并保存下来

    ref_basename = os.path.splitext(os.path.basename(file_ref))[0]  # 去掉路径和扩展名
    file_seqs_path = file_seqs + '.forcluster'
    file_ref_path = file_ref + '.forcluster'
    ...
    print(file_ref_path, file_seqs_path)
    print(os.getcwd())

    # commands = f"""
    # cd ./polls/Code/bwa
    # ./bwa index "{file_ref_path}"
    # ./bwa mem -t 16 "{file_ref_path}" "{file_seqs_path}" > "{dir}{ref_basename}_ali.sam" 2> "{dir}{ref_basename}_bwa_mem.log"
    # cd {dir}
    # cat "{ref_basename}_ali.sam" | samtools view -bS - > "{ref_basename}_output.bam"
    # samtools sort -@ 16 "{ref_basename}_output.bam" -o "{ref_basename}_output_sorted.bam"
    # samtools index "{ref_basename}_output_sorted.bam"
    # samtools view "{ref_basename}_output_sorted.bam" | cut -f 3,4,5,6,10,11 > "{ref_basename}_cluster.ali"
    # """
    # commands = f"""
    #
    # ./polls/Code/bwa/bwa index "{file_ref_path}"
    # ./polls/Code/bwa/bwa mem -t 16 "{file_ref_path}" "{file_seqs_path}" > "{dir}{ref_basename}_ali.sam" 2> "{dir}{ref_basename}_bwa_mem.log"
    #
    # cat "{dir}{ref_basename}_ali.sam" | ./polls/Code/samtools/src/samtools-1.18/install/bin/samtools view -bS - > "{dir}{ref_basename}_output.bam"
    # ./polls/Code/samtools/src/samtools-1.18/install/bin/samtools sort -@ 16 "{dir}{ref_basename}_output.bam" -o "{dir}{ref_basename}_output_sorted.bam"
    # ./polls/Code/samtools/src/samtools-1.18/install/bin/samtools index "{dir}{ref_basename}_output_sorted.bam"
    # ./polls/Code/samtools/src/samtools-1.18/install/bin/samtools view "{dir}{ref_basename}_output_sorted.bam" | cut -f 3,4,5,6,10,11 > "{dir}{ref_basename}_cluster.ali"
    # """
    commands = f"""
        ...
        cat "{dir}{ref_basename}_ali.sam" | samtools view -bS - > "{dir}{ref_basename}_output.bam"
        samtools sort -@ 16 "{dir}{ref_basename}_output.bam" -o "{dir}{ref_basename}_output_sorted.bam"
        samtools index "{dir}{ref_basename}_output_sorted.bam"
        samtools view "{dir}{ref_basename}_output_sorted.bam" | cut -f 3,4,5,6,10,11 > "{dir}{ref_basename}_cluster.ali"
        """
    ...
    print(result.stdout)
    print(f"引入比对时间：{str(datetime.now() - starttime1)}")
    # print(result.stderr)
    print("hello")
    cluster_ali_path = os.path.join(dir, f"{ref_basename}_cluster.ali")  # 统一命名

    if str(file_seqs).endswith('fastq'):
        mdict = read_cluster_ali_phred(cluster_ali_path, sequence_length)
    else:
        mdict = read_cluster_ali_nophred(cluster_ali_path, sequence_length)
    simuseqs,simuquas = [],[]
