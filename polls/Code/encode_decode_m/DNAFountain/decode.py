...
class Glass:
    ...
    def add_dna(self, dna_string):
        ...
        if self.rs > 0:
            ...
            if self.correct:  # we want to evaluate the error correcting code
                ...
                # path=os.path.join(os.path.dirname(self.out), 'rs_corrected_dna.txt')
                # with open(path, "a") as f:  # 你可以改文件名
                #     bin_str = ''.join(format(b, '08b') for b in data_corrected)
                #     if len(bin_str) % 2 != 0:
                #         bin_str += '0'
                #     base_map = {'00': 'A', '01': 'C', '10': 'G', '11': 'T'}
                #     dna_seq = ''.join(base_map[bin_str[i:i + 2]] for i in range(0, len(bin_str), 2))
                #     f.write(dna_seq + '\n')  # 每段一行
                ...
                # if numpy.count_nonzero(data != list(data_again)) > self.rs/2:
    ...
    def write_chunks_as_dna(self, filename):
        """
        将 self.chunks 中每个 chunk 转换为 DNA 碱基序列，每个写入一行。
        编码方式：2-bit -> 1 base (00->A, 01->C, 10->G, 11->T)
        """
        base_map = {'00': 'A', '01': 'C', '10': 'G', '11': 'T'}

        with open(filename, 'w') as f:
            for chunk in self.chunks:
                # 1. 把 chunk 中的每个字节转为8位二进制字符串
                bin_str = ''.join(format(b, '08b') for b in chunk)

                # 2. 补0使长度是2的倍数（避免最后一位不完整）
                if len(bin_str) % 2 != 0:
                    bin_str += '0'
...
class Decode():
    ...
    def main(self, crc_num, decision='hard', phreds=[]):
        ...
        if not glass.isDone():
            logging.error("Could not decode all file...")
            sys.exit(1)
        clean_out = os.path.dirname(self.out)
        # glass.write_chunks_as_dna(os.path.join(clean_out, 'chunks.txt'))
