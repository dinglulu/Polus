...
class DNAFountainEncode(AbstractEncode):
    ...
    def getgc(self,all_seqs):
        ...
        for dna_sequence in all_seqs:
            ...
            avggc += gc
            all_gc.append(gc)
        # with open(f'gcfiles/gc_{self.tool_name}_{self.file_base_name}.csv', mode='w', newline='') as file:
    ...
    # def _set_param_common(self,method='fountain',real_gc=0.5,real_max_homopolymer=5,density=1.6,encode_time=0.5):
    ...
    #     return param
    def _set_param_common(
            self,
            method='fountain',
            real_gc=0.5,
            real_max_homopolymer=5,
            density=1.6,
            encode_time=0.5,

    ):
        param = (
            "method:{},seq_len:{},gc_bias:{},max_homopolymer:{},rs_num:{},crc_num:{},"
            "redundancy:{},delta:{},c_dist:{},chunk_num:{},Encoding Runtime:{},"
            "Logical Density:{},GC Contents:{},Maximum Homopolymers Length:{}\n"
        ).format(
            method,
            self.codec_param.sequence_length,
            self.gc_bias,
            self.codec_param.max_homopolymer,
            self.codec_param.rs_num,
            self.crc_num,
            self.redundancy,
            self.delta,
            self.c_dist,
            ...
            encode_time,  # Encoding Runtime


            density,  # Logical density (同上，具体你可以换成逻辑密度变量)
            real_gc,  # GC contents
            real_max_homopolymer,  # Maximum homopolymers length
    ...
    def common_encode(self):
        ...
        self.encode_time = (datetime.now() - tm_encode).total_seconds()

        with open(encodefile, 'r', encoding='utf-8') as f:
            old_content = f.readlines()
        dnaseqs = [old_content[i].strip('\n') for i in range(len(old_content))]
        real_gc = self.getgc(dnaseqs)
        real_max_homopolymer = self.max_homopolymer_length(dnaseqs)
        self.total_base = len(dnaseqs) * len(dnaseqs[0])

        self._set_density()

        param = self._set_param_common('fountain',real_gc,real_max_homopolymer,self.density,self.encode_time)
...
class DNAFountainDecode(AbstractDecode):
    ...
    def common_decode(self):
        ...
        try:
            ...
            infos = {'code': 1}
            print(f"------解码失败------")
            infos['decode_time'] = (datetime.now() - tm_encode).total_seconds()
            return infos
        except ValueError:
            infos = {'code': 1}
            print(f"------解码成功，比对失败------")
...
class DerrickEncode(AbstractEncode):
    ...
    def _set_param_common(self,method='derrick'):
        param = "method:{},seq_len:{},matrix_n:{},matrix_r:{},Encoding Runtime:{},Logical Density:{},GC Contents:{},Maximum Homopolymers Length:{}\n".format(
            method,self.codec_param.sequence_length,self.matrix_n,self.matrix_n - self.matrix_k,self.encode_time,self.density,self.gc,self.max_homopolymer)
        return param
    ...
    def common_encode_matrix(self):
        tm_encode = datetime.now()
        encodefile = self.output_file_path
        now_path=os.path.join(os.getcwd(), 'polls/Code/encode_decode_m')
        ...
        shell = f"{now_path}/derrick/derrick encode -i {now_path}/derrick/pi.txt -n {self.matrix_n} -k {self.matrix_k} -s {self.codec_param.sequence_length//4}" \
                f" {self.input_file_path} >{self.output_file_path}"
...
class DerrickDecode(AbstractDecode):
    def __init__(self, input_file_path:str, output_dir:str,inputfileforcompare='',matrix_code=False,matrix_n=0,matrix_r=0,sequence_length=260):
        ...
        input_ext = os.path.splitext(self.inputfileforcompare)[1]

        # 构造输出路径
        self.output_file_path = os.path.join(
            self.output_dir,
            f"{self.file_base_name}_decode{input_ext}"
        )
    ...
    def common_decode_matrix(self):
        ...
        with open(self.input_file_path, 'w', encoding='utf-8') as f:
            for i in range(len(dnaseqs)):
                f.write(f">seq{i}\n{dnaseqs[i]}\n")
        print(os.getcwd())
        shell = f"./polls/Code/encode_decode_m/derrick/derrick decode -i ./polls/Code/encode_decode_m/derrick/pi.txt -n {self.matrix_n} -k {self.matrix_k}  -s {self.sequence_length//4}" \
                f"  {self.input_file_path} -o {self.output_file_path}  "
        print(f"------开始解码------")
        result = subprocess.run(shell, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        print(f"shell:{shell}\nresult.stderr:{result.stderr}\n------解码结束------\n")
        infos = self.judgedrrick()
        infos['decode_time'] = (datetime.now() - tm_decode).total_seconds()
        return infos
...
class PolarEncode(AbstractEncode):
    def __init__(self, input_file_path:str, output_dir:str, frozen_bits_len:int,):
        ...
        file_name = os.path.basename(self.input_file_path)  # input.fasta
        file_base_name, _ = os.path.splitext(file_name)  # input

        self.file_base_name = file_base_name
        self.output_file_path = os.path.join(
            output_dir,
            f"{self.file_base_name}_{self.tool_name}.fasta"
        )
    ...
    def _set_param_common(self,method='PolarCode'):
        param = "method:{},seq_len:{},homopolymer:{},frozen_bits_len:{},Encoding Runtime:{},Logical Density:{},GC Contents:{},Maximum Homopolymers Length:{}\n".format(
            method,260,4,self.frozen_bits_len,self.encode_time,
        self.density, self.gc, self.max_homopolymer)
        return param
...
class PolarDecode(AbstractDecode):
    def __init__(self, input_file_path: str, orifile: str, output_dir: str,matrices_ori,matrices_dna_ori,matrices_01_ori,frozen_bits_len=5):
        ...
        file_name = os.path.basename(self.orifile)
        file_base_name, file_ext = os.path.splitext(file_name)

        self.file_base_name = file_base_name
        self.output_file_path = os.path.join(
            self.output_dir,
            f"{self.file_base_name}_decode{file_ext}"
        )
    ...
    def common_decode(self):
        ...
        try:
            ...
            run_time = (datetime.now() - tm_decode).total_seconds()
...
class HedgesEncode(AbstractEncode):
    def __init__(self, input_file_path:str, output_dir:str, sequence_length:int, max_homopolymer=4,
                 rs_num=0, add_redundancy=False, add_primer=False, primer_length=20,
    ...
    def _set_param_common(self,method='hedges'):
        ...
        param+="Encoding Runtime:{},Logical Density:{},GC Contents:{},Maximum Homopolymers Length:{}\n".format(self.encode_time,self.density, self.gc, self.max_homopolymer)
        return param
    ...
    def _encode(self, bit_segments,coderatecode):
        ...
        # activate_command = "conda run -n hedges python ./polls/Code/encode_decode_m/hedges_encode_decode.py encode files/bit_segments.txt"

        base_name = os.path.splitext(os.path.basename(self.output_file_path))[0]
        file_name = f"{base_name}_bit_seg.txt"

        bit_segments_path = os.path.join(self.output_dir, file_name)

        # 写入 bit_segments
        writemessplain(bit_segments_path, bit_segments)
        # conda
        # run - n
        # hedges
        # 构建命令
        activate_command = (
            f"conda run -n hedges python ./polls/Code/encode_decode_m/hedges_encode_decode.py "
            f"encode {bit_segments_path}"
        )
...
class HedgesDecode(AbstractDecode):
    ...
    def common_decode_matrix(self):
        ...
        print("out_file_path1:", self.output_file_path)
        ...
        self.run_time = (datetime.now() - tm_run).total_seconds()
    ...
    def _decode(self, segmentfor_decode):
        ...
        base_dir = os.path.dirname(self.output_file_path)
        stem = os.path.splitext(os.path.basename(self.output_file_path))[0]

        outfilepath = os.path.join(base_dir, f"{stem}_bit_segments_decode.txt")
        fordecode_path = os.path.join(base_dir, f"{stem}_bit_segments_fordecode.fasta")

        self.writemessplain(fordecode_path, segmentfor_decode)
        ...
        # activate_command = "conda run -n hedges python polls/Code/encode_decode_m/hedges_encode_decode.py decode bit_segments_fordecode.fasta"

        activate_command = (
            f"python2 polls/Code/encode_decode_m/hedges_encode_decode.py "
            f"decode {fordecode_path} {outfilepath} {self.totstrandlen} {self.coderatecode} "
            f"{self.max_hpoly_run} {self.npackets}"
        )
...
class YinYangCodeEncode(AbstractEncode):
    ...
    def _set_param_line(self, left_primer="", right_primer=""):
        param = "totalBit:{},binSegLen:{},leftPrimer:{},rightPrimer:{},fileExtension:{},bRedundancy:{},RSNum:{},".format(
    ...
    def _set_param_common(self,method='YYC'):
        ...
        param += self._set_real_param(self.gc, self.max_homopolymer,
                                      self.density, self.encode_time)
        return param
    def _set_real_param(self,real_gc, real_max_homopolymer, density, encode_time):
       param="Encoding Runtime:{},Logical Density:{},GC Contents:{},Maximum Homopolymers Length:{}\n".format(encode_time,density,real_gc,real_max_homopolymer)
       return param
    def _encode(self, bit_segments,coderatecode,max_content):
        ...
        return yinyangencode(bit_segments,self.codec_param.max_homopolymer,max_content)
    ...
    def common_encode(self):
        ...
        #         file.write(str(data) + '\n')
        # output_path = os.path.join(self.output_dir, 'messplains.txt')
        ...
        # encode
        tm_encode = datetime.now()
        base_segments = self._encode(bit_segments, len(bin_str),max_content)
...
class YinYangCodeDecode(AbstractDecode):
    ...
    def common_decode(self):
        ...
        info=self.judgedrrick()
        self.infos.update(info)

        self.infos['decode_time'] = (datetime.now() - tm_decode).total_seconds()
        return self.infos
