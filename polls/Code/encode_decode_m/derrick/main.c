...
int main_encode(int argc, char **argv)
{
    ...
    // 假设ef是你的数据结构，下面是你给的循环
    for(i = 0; i < ef->nblocks * ef->n; i++) {
        j = i + 1;
        
        // 同时输出到屏幕和文件
        fprintf(stdout, ">contig%d\n", j);
        fprintf(output_file, ">contig%d\n", j);  // 写入文件

        for(j = 0; j < ef->tlen; j++) {
            switch(ef->tseq[i * ef->tlen + j]) {
                case 1:
                    fprintf(stdout, "C");
                    fprintf(output_file, "C");  // 写入文件
                    break;
                case 2:
                    fprintf(stdout, "G");
                    fprintf(output_file, "G");  // 写入文件
                    break;
                case 3:
                    fprintf(stdout, "T");
                    fprintf(output_file, "T");  // 写入文件
                    break;
                default:
                    fprintf(stdout, "A");
                    fprintf(output_file, "A");  // 写入文件
                    break;
            }
        }
...
int usage_decode()
{
	...
	" -f <int>    shift or not, [1]\n"
        " -v          verbose\n"
		" -o <file>   指定输出文件名（默认 decode_output.txt）\n"
	"\n"
...
int main_decode(int argc, char **argv)
{
	...
	shift_or_not = 1;
	ver = 0;
	while((c = getopt(argc, argv, "hi:n:k:s:M:X:O:E:c:d:m:j:u:l:r:t:a:R:b:e:f:v:o:")) != -1)
	{
                switch(c)
		{
			case 'i':
	...
	if(df->begblock == 1 && df->endblock == df->nblocks)
{
	...
	{
    fprintf(stderr, "❌ 未指定输出文件名，请使用 -o 参数指定\n");
    return 1;
	}
    ...
    // FILE *output_file = fopen("decode_output.txt", "w");
    // if (output_file == NULL) {
    //     perror("无法打开文件");
